...
module ::DiscoursePrivateReplies
  def DiscoursePrivateReplies.can_see_all_posts?(user, topic)
    ...
    if (
         SiteSetting.private_replies_groups_can_see_all.split("|").map(&:to_i) &
    ...
    if SiteSetting.private_replies_topic_starter_primary_group_can_see_all && topic
      groupids = Group.find(topic.user.primary_group_id).users.pluck(:id) if topic.user &&
        !topic.user.anonymous?
  ...
  def DiscoursePrivateReplies.can_see_post_if_author_among(user, topic)
    userids = []
    Group
      .where("id in (?)", SiteSetting.private_replies_see_all_from_groups.split("|"))
  ...
  reloadable_patch do
    ...
    module ::PostGuardian
      ...
      def can_see_post?(post)
        ...
        if SiteSetting.private_replies_enabled &&
             post.topic.custom_fields.keys.include?("private_replies") &&
             post.topic.custom_fields["private_replies"]
    ...
    module PatchTopicView
      def participants
        result = super
        if SiteSetting.private_replies_enabled &&
             @topic.custom_fields.keys.include?("private_replies") &&
             @topic.custom_fields["private_replies"]
    ...
    module PatchTopicViewDetailsSerializer
      def last_poster
        if SiteSetting.private_replies_enabled &&
             object.topic.custom_fields.keys.include?("private_replies") &&
    ...
    module PatchSearch
      def execute(readonly_mode: @readonly_mode)
        ...
        if SiteSetting.private_replies_enabled &&
             !DiscoursePrivateReplies.can_see_all_posts?(@guardian.user, nil)
          userids = DiscoursePrivateReplies.can_see_post_if_author_among(@guardian.user, nil)

          protected_topics =
            TopicCustomField.where(name: "private_replies").where(value: true).pluck(:topic_id)
        ...
        def apply_common_filters(builder, user_id, guardian, ignore_private_messages = false)
          if SiteSetting.private_replies_enabled &&
            ...
            if !protected_topic_list.empty?
              builder.where(
                "( (a.target_topic_id not in (#{protected_topic_list})) OR (a.acting_user_id = t.user_id) OR (a.acting_user_id in (#{userid_list})) )",
              )
            end
          end
    ...
    DiscourseEvent.on(:topic_created) do |topic|
      if SiteSetting.private_replies_enabled
        if (SiteSetting.private_replies_on_selected_categories_only == false) ||
             (topic&.category&.custom_fields&.dig("private_replies_enabled"))
